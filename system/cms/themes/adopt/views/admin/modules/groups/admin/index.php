			...
			<table class="table-list" cellspacing="0">
				...
				<?php foreach ($groups as $group):?>
					<tr>
						...
						<td class="">
						<?php echo anchor('admin/groups/edit/'.$group->id, lang('buttons:edit'), 'class="btn btn-primary"') ?>
						<?php if ( ! in_array($group->name, array('user', 'admin'))): ?>
							<?php echo anchor('admin/groups/delete/'.$group->id, lang('buttons:delete'), 'class="confirm btn btn-danger"') ?>
						<?php endif ?>
						<?php echo anchor('admin/permissions/group/'.$group->id, lang('permissions:edit'), 'class="btn btn-primary"') ?>
						</td>
					</tr>
				<?php endforeach;?>
				</tbody>
